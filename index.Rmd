...
```{r}
#test shape file - Detroit Council Districts

#blight violations district? -> see high concentration of blight in East and West Detroit, coincides with large number of demolitiosn in those areas

f <- 'geo_export_c0cba125-2fb0-432f-9a11-e6bb3c7dd89e.shp'
#b <- 'geo_export_8edb7010-386f-4605-9edf-dd1633730b76.shp'
districts_shp <- st_read(here('detroit_data','City Council Districts',f))
#detroit_boundary <- st_read(here('detroit_data','City of Detroit Boundary',b))
...
ggplot(data = districts_shp) +
  geom_sf(fill = '#31044F', color = 'white') + 
  ...
  #coord_sf(xlim = c(-86, -82.9), ylim = c(42.25, 45), expand = FALSE) +
  #map_theme +
  ...
  annotate('text', x = -83, y = 42.27, label = '*Each orange dot represents a blight violation', size=2.5)
...
```{r}
#test shape file - Detroit Neighborhoods

#plot demolitions by neighborhood?

f <- 'geo_export_30086289-81b8-46bd-bbf7-9d8d3ae67dce.shp'

neighbor_shp <- st_read(here('detroit_data','Detroit Neighborhoods',f))

past_demolitions %<>% mutate(nhood_name = Neighborhood)
x<-left_join(neighbor_shp, past_demolitions, by = c('nhood_name'))


ggplot(data = neighbor_shp) +
  geom_sf(fill = 'blue')
