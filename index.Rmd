...
blight_map <- ggplot() +
  ...
  geom_point(data = dots, aes(x=X, y=Y), size = 0.25, color = '#ED9007', fill = "#ED9007", alpha=0.5) +
...
To maintain publically-owned blighted and abandoned land, Detroit has one of the largest demolitions programs in the country. The city spends tens of millions of dollars each year to demolish thousands of buildings. In 2018, council districts 4 and 7 (east and west Detroit respectively) had the highest number of demolitions. As shown above, they also had the highest concentration of blight that year. 
...
#cent <- left_join(st_centroid(districts_shp), sym, by = 'districts')

dem_map <- ggplot() +
  #geom_sf(data = districts_shp, fill = '#31044F', color = 'white') + 
  geom_sf(data = districts_shp, fill = '#31044F', color = 'white') + 
  #geom_sf(data= cent, aes(size=count), color = '#ED9007') +
  geom_point(data = sym, aes(x=Longitude, y=Latitude), color = '#8C82FA', alpha=0.5) +
  ...
  annotate('text', x = -83.325, y = 42.42, label = 'District 1', color = 'black', size =3, family = my_theme$text[['family']], fontface='bold') +
    ...
    coord_sf(xlim = c(-83.4, -82.85), ylim = c(42.25, 42.47), expand = FALSE) +
    map_theme +
    labs(title = 'High Concentration of Demolitions in Western and Eastern Detroit', subtitle = '2018 Demolitions in Detroit', caption = 'Data Source: Detroit Open Data Portal') +
...
```{r Demolition comparison, warning=F, fig.align='center', echo=F}
past_demolitions %<>% 
  mutate(`Demolition Date` = as.Date(`Demolition Date`,format = "%m/%d/%Y")) %>%
  mutate(month_year = `Demolition Date`) %>%
  mutate(year = as.numeric(strftime(`Demolition Date`, '%Y'))) %>%
...
hist2 <- ggplot(data=non_com) + 
  ...
  my_theme +
  scale_y_continuous(expand=c(0,0), limits= c(-1000,3950), breaks=seq(0,3000,1000), labels=comma) + 
  #scale_x_continuous(expand=c(0,0), limits = c(0,85000), labels=comma) +
  scale_x_continuous(expand=c(0,0), labels=comma, limits = c(0,85000)) +
