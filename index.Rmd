...
#theme
my_theme <- theme_classic() + theme(text = element_text(family="Helvetica", color = 'black'), 
...
The Hardest Hit Funding program and is the largest of the city’s funding sources for its demolition program, which, by law, can be spent only in federally-designated areas of the city. The funds are split across the city to carry out demolitions. In 2018, council districts 4 and 7 (east and west Detroit respectively) had the highest number of demolitions funded by HHf. In some instances, a house will switch to non-HHF or have city funds used to cover additional costs over the program limit of $25,000. 
...
ggplot() +
  ...
  geom_text(data = circle_data, aes(x, y, size=count, label = Council_District), color = 'white', size=3.5) +
  ...
  theme(legend.position="none", axis.line = element_line(color='white'), 
        ...
        plot.subtitle=element_text(hjust=0.5), plot.caption = element_text(hjust=2)) +
  labs(title = 'Districts 4 and 7 Had The Highest Number \nof Hardest Hit Funding Demolitions in 2018', subtitle = 'HHF Demolitions in Detroit by Council District', caption = 'Data source: Detroit Open Data Portal',x='',y='') +
...
side_lot_sales %>%
  ...
  my_theme + theme(legend.position = c(0.92,0.9)) + scale_x_continuous(expand=c(0,0), lim = c(2015,2018.02)) +
...
##Programs to Sell City-Owned Land
...
ggplot(all_three, aes(x=year, y=count)) + 
  ...
  my_theme + 
  theme(strip.text = element_text(face='bold', color = '#ED9007', size=10), strip.background = element_rect(color='white')) +
  scale_y_continuous(labels=comma) +
  labs(title = 'Auctions and "Own It Now" Sales Are Increasing,\nFall Behind Side Lot Sales', subtitle = 'Properties Sold through Detroit Land Bank Authority Programs Each Year', caption = 'Data Source: Detroit Open Data Portal', x='Year', y='Number of Properties Sold')
...
Despite increased sales through Auctions and the "Own It Now" program, buildup of city-owned land that is not for sale is often a frustrating barrier for those interested in buying property in the city. It is worth noting that a large portion of the Detroit Land Bank Authority's inventory is categorized as Owned Vacant Land as opposed to property that is for sale to private owners. There is a particularly high number of Owned Vacant Land properties compared to other inventory types in District 5, which is the downtown Detroit area.
