...
While blight plagues the entire city, it is more concentrated in some areas than others. The southern tip of District 7 (West Detroit) and the eastern part of District 4 (East Detroit) had a highly concentrated number of blight tickets issued in 2018. As these areas are particularly troubled by blight, we will see later that the city has concentrated demolition efforts there as well.
...
ggplot() +
  ...
  geom_point(data = dots, aes(x=X, y=Y), size = 0.25, color = '#ED9007', fill = "#ED9007", alpha=0.75) +
...
high <- non_com %>% filter(Price > 60000) %>% mutate(x=0.5)

med <- median(non_com$Price)

den <- ggplot(data = non_com) + 
  #geom_line(data = non_com, aes(x=Price, fill = Price), stat='density') +
  geom_density(aes(x=Price), fill = '#31044F', alpha = 0.95) +
  geom_vline(xintercept = med, linetype="dashed", color = "#ED9007") +
  #geom_histogram(aes(x=Price, fill = ..x..)) + scale_fill_gradient(low="#31044F", high="#ED9007") +
  my_theme +
...
box <- ggplot() +
  geom_boxplot(data = non_com, aes(y=Price), color = "#ED9007", fill = "#31044F") + 
  #geom_hline(yintercept = med, linetype = 'dashed',color = "#ED9007") +
  coord_flip() + 
  #scale_x_continuous(expand=c(0,0), limits = c(0,1)) + 
  scale_y_continuous(expand=c(0,0), limits = c(0,80000)) +
  #geom_point(data=non_com, aes(x=demolitions, y=Price))
  #geom_jitter() +
  my_theme +
...
hist <- ggplot(data=non_com) + geom_histogram(aes(x=Price), fill = "#31044F") + my_theme +
  scale_y_continuous(expand=c(0,0), limits= c(-1000,3000), labels=comma) + scale_x_continuous(expand=c(0,0), limits = c(0,85000), labels=comma) +geom_vline(xintercept = med+100, linetype="dashed", color = "#ED9007")
...
side_lot_sales %>%
  ...
  labs(title = 'Claytown Has the Largest Number of Side Lot Sales,\nFollowed by Airport Sub, Brightmoor, And Chadsey Condon ', subtitle ='Side Lot Sales by Neighborhood since 2015', x='Year', y='Number of Side Lots Sold', caption='Data Source: Detroit Open Data Portal') 
...
ggplot(all_three, aes(x=year, y=count)) + 
  ...
  scale_y_continuous(expand=c(0,0), labels=comma) + scale_x_continuous(expand=c(0,0)) +
